 ...
 */
  ...
  class XmlScriptletTest extends TestCase {
    ...
    protected function newRequest($method, URL $url) {
      $q= $url->getQuery('');
      $req= new XMLScriptletRequest(new HttpScriptletRequest());
      $req->setMethod($method);
      $req->putEnvValue('SERVER_PROTOCOL', 'HTTP/1.1');
      $req->putEnvValue('REQUEST_URI', $url->getPath('/').($q ? '?'.$q : ''));
      $req->putEnvValue('QUERY_STRING', $q);
      $req->putEnvValue('HTTP_HOST', $url->getHost());
      if ('https' === $url->getScheme()) {
        $req->putEnvValue('HTTPS', 'on');
      }
