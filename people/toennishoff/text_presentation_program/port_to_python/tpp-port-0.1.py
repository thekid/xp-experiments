...
TODO:
  ...
  - Wrap all output method calls in a class
...
CHANGELOG:
  V 0.1 17.03.2009:
    Start of translation to Python 3.0
'''

'''
#!/usr/bin/env ruby

version_number = "1.3.1" (Ruby base version ported)
'''


# Version Number of Python port
version_number= "0.1";
...
standarderror= '''An unforseen Error occured.
Please report this Bug and as much related information as posible to the developers
at tpp@steyner-x.de
'''
...
# Kept for completeness ncurses is part of the Python standard library.
# Future: use a different library for windows and wrap all output calls into a class 
# to be truly platform independent
def load_ncurses():
  '''
    Loads the ncurses-ruby module and imports "Ncurses" into the
    current namespace. It stops the program if loading the
  ...
  try:
    ...
    print 'There is no Ncurses package installed which is needed by TPP.'
    sys.exit(1)
  except:
    print standarderror
    print 'Error importing curses library.'
    sys.exit(1)
...
class ColorMap:
  ...
  def get_color(color):
    ...
    # Wie funktioniert curses in Python?
    # 
    colors= {
      'white'   : '',
      'yellow'  : '',
...
class FileParser:
  ...
  def get_pages():
    ...
    print standarderror
    print 'Error opening presentation file.'
      sys.exit(1)

    number_pages= 0
    cur_page=     Page("slide " + (number_pages + 1))
    ...
    for line in infile:
      ...
      if (line.startswith('--#')):
        ...
        number_pages+= 1
        name = line.lstrip('--newpage','')
        if (name == ""):
          name = "slide " + (number_pages+1)
        else:
          name.strip()
        end
        cur_page = Page(name)
...
class Page:
  ...
  def next_line():
    ...
    self.cur_line+= 1
    if (self.cur_line >= self.lines.size):
      self.eop= True
    return line
