 ...
 */
  ...
  class Type extends Object {
    ...
    public static function forName($name) {
      ...
      // * Deprecated: XP < 5.8 used __generic field
      // * Deprecated: array<T> is T[], array<K, V> is [:T]
      $base= substr($type, 0, $p);
      $components= self::forNames(substr(rtrim($type, '()'), $p+ 1, -1));
      if ('array' !== $base) {
        $class= cast(self::forName($base), 'lang.XPClass');
        if ($class->hasField('__generic')) {
          return XPClass::forName('lang.GenericsBC')->newInstance($class, $components);
        } else {
          return $class->newGenericType($components);
        }
      }
     ...
     *
     * @throws  lang.IllegalStateException if the type cannot be instantiated
     * @throws  lang.IllegalArgumentException if the value given to instantiate this type is incorrect
     */
    public function newInstance() {
      throw new IllegalStateException('The '.$this->name.' type cannot be instantiated.');
    }
  }
?>
