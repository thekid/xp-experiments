 ...
 */
  ...
  uses(
    ...
    'web.http.HttpScriptlet',
    'web.http.HttpScriptletRequest',
    'web.http.HttpScriptletResponse',
    'web.ScriptletOutputStream'
  );

  /**
   * Base class for HTTP scriptlet tests
   *
   * @purpose  Abstract base class
   */
  abstract class AbstractHttpScriptletTestCase extends TestCase {
    ...
    protected function newHttpRequest($method, URL $url) {
      return newinstance('web.http.HttpScriptletRequest', array($method, $url), '{
        private $url, $method;
    ...
    protected function newHttpResponse(OutputStream $stream) {
      return newinstance('web.http.HttpScriptletResponse', array($stream), '{
        private $stream;
        private $status= 200;
